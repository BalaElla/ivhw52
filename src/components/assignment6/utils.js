...
function groupByCity(data) {
    ...
    let result = data.reduce((acc, d) => {
        ...
        let destCity = acc[d.DestCity] || {
            "City": d.DestCity,
            "Count": 0
        };
        destCity.Count += 1;
        acc[d.DestCity] = destCity;

        // Handle the source city, check if it is the same as destination city to avoid double counting
        if (d.DestCity !== d.SourceCity) {
            let sourceCity = acc[d.SourceCity] || {
                "City": d.SourceCity,
                "Count": 0
            };
            sourceCity.Count += 1;
            acc[d.SourceCity] = sourceCity;
        }
        return acc;
    }, {});

    // Convert the dictionary to an array and sort by count
    result = Object.values(result).sort((a, b) => b.Count - a.Count);
    return result;
}
